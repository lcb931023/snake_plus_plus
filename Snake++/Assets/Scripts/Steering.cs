/***************Steering***************
 * Haz basic steering behavior functionalities
 * 
 **************************************/
using UnityEngine;
...
public class Steering : MonoBehaviour
{
	...
	public float maxForce = 50.0f;	// maximimum force allowed
	...
	/*
	public Vector3 SnakeFollow ( Vector3 targetPos, float followDistance)
	{
		//find dv, desired velocity
		Vector3 dv = targetPos - transform.position;
		//change the seeking point to the end of that object
		dv *= (1- followDistance/dv.magnitude);
		dv = dv.normalized * maxSpeed;
		dv -= transform.forward * speed;
		return dv;
	}
	...
	// same as seek pos above, but parameter is game object
	public Vector3 SnakeFollow ( GameObject go, float followDistance)
	{
		return SnakeFollow ( go.transform.position, followDistance);	
	}	
	*/
	public Vector3 SnakeFollow ( Vector3 targetPos, float followDistance)
	{
	...
	public Vector3 StayInBound ( Vector3 areaCentre, float areaRadius ){
		if(Vector3.Distance(transform.position, areaCentre) > areaRadius){
			return Seek (areaCentre);
		}else{
			return Vector3.zero;
		}
	}
